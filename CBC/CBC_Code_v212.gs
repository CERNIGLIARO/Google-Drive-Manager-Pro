...
function frParserConstructCenter_(text) {
  ...
  // Normalise les références même si l'OCR a ajouté des espaces autour des "/".
  raw = raw
    .replace(/(R?INV)\s*\/\s*(\d{4})\s*\/\s*([A-Z0-9]+)/gi, '$1/$2/$3')
  ...
  function parseZone_(zoneText, refPattern, section, forcedType) {
    ...
    while ((rm = refRe.exec(zoneText)) !== null) {
      refs.push({ ref: String(rm[1] || '').toUpperCase(), start: rm.index, endRef: refRe.lastIndex });
  ...
  const bnkMatch = raw.match(/(?:^|\n)\s*BNK1\/\d{4}\/\d{2}\/\d{4}\b/im);
  const nonEchuMatch = raw.match(/(?:^|\n)\s*Non\s+(?:échu|echu)\s*(?:\n|$)/im);
  ...
  if (firstBnkPos >= 0) {
    ...
    parseZone_(raw.slice(0, firstBnkPos), '(?:R?INV)\\/\\d{4}\\/[A-Z0-9]+', 'ÉCHU', '');
    ...
    if (nonEchuEnd >= 0) {
      parseZone_(raw.slice(nonEchuEnd), '(?:R?INV)\\/\\d{4}\\/[A-Z0-9]+', 'NON ÉCHU', '');
    }
  } else {
    // Variante sans bloc BNK1.
    if (nonEchuPos >= 0) {
      parseZone_(raw.slice(0, nonEchuPos), '(?:R?INV)\\/\\d{4}\\/[A-Z0-9]+', 'ÉCHU', '');
      parseZone_(raw.slice(nonEchuEnd), '(?:R?INV)\\/\\d{4}\\/[A-Z0-9]+', 'NON ÉCHU', '');
    } else {
      parseZone_(raw, '(?:R?INV)\\/\\d{4}\\/[A-Z0-9]+', 'ÉCHU', '');
    }
  }
...
function frEcrireFacturesAPayer_(ss, parsed, result) {
  ...
  sh.getRange('G3').setValue('Paiements retrouvés').setFontWeight('bold');
  ...
  sh.getRange('A4:R4').merge()
    .setValue('Le "reste à payer" est calculé facture par facture à partir des paiements bancaires rapprochés. Les avoirs et ajustements du relevé fournisseur sont affichés séparément et ne sont pas automatiquement affectés à une facture : contrôle conseillé avant paiement.')
  ...
  const headers = [
    ...
    'Date facture','Échéance','Montant facture','Paiements trouvés','Reste à payer',
