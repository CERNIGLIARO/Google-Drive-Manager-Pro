...
function ecrireResultatsRecherche_(ss, results, criteria, maxResults) {
  ...
  // Totaux des résultats de recherche (ligne 3)
  // On écrit des valeurs calculées par Apps Script plutôt que des formules,
  // afin d'éviter les problèmes de séparateur décimal selon la locale.
  const totalEntrees = results.reduce((sum, r) => sum + (r.amount > 0 ? r.amount : 0), 0);
  const totalSorties = results.reduce((sum, r) => sum + (r.amount < 0 ? Math.abs(r.amount) : 0), 0);
  ...
  sheet.getRange('J3').setValue(totalEntrees);
  sheet.getRange('K3').setValue(totalSorties);
  sheet.getRange('L3').setValue('DIFFÉRENCE').setFontWeight('bold').setHorizontalAlignment('right');
  sheet.getRange('M3').setValue(difference);
  sheet.getRange('J3:K3').setNumberFormat('#,##0.00').setFontWeight('bold');
  sheet.getRange('M3').setNumberFormat('#,##0.00').setFontWeight('bold');
  sheet.getRange('I3:M3').setBackground('#f1f3f4');
