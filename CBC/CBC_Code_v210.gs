...
function frParserConstructCenter_(text) {
  ...
  const ibanMatch = String(text || '').replace(/\s+/g, ' ').match(/Banque\s*:\s*(BE(?:\s*\d){14})/i);
  let supplierIban = ibanMatch ? normaliserCompteRecherche_(ibanMatch[1]) : '';
  if (!supplierIban) {
    const any = String(text || '').match(/BE(?:\s*\d){14}/i);
    if (any) supplierIban = normaliserCompteRecherche_(any[0]);
  }

  const reportM = String(text || '').match(/(?:À|A)\s+la\s+date\s+du\s+(\d{1,2}\/\d{1,2}\/\d{4})/i);
  const reportDate = reportM ? convertirDate_(reportM[1]) : null;

  let clientName = '';
  for (let i=0;i<lines.length;i++) {
    if (/^Solde$/i.test(lines[i])) {
      for (let j=i+1;j<Math.min(lines.length,i+6);j++) {
        if (/^(Échu|Echu|Non échu|Non echu)$/i.test(lines[j])) continue;
        if (!frEstDateTexte_(lines[j]) && !frEstMontantTexte_(lines[j]) && !/^\d{4}-\d{2}-\d{2}/.test(lines[j])) {
          clientName = lines[j]; break;
        }
      }
      if (clientName) break;
    }
  ...
  for (let i=0;i<lines.length;i++) {
    const line = lines[i];
    if (/^Échu$/i.test(line) || /^Echu$/i.test(line)) { section = 'ÉCHU'; continue; }
    if (/^Non\s+échu$/i.test(line) || /^Non\s+echu$/i.test(line)) { section = 'NON ÉCHU'; continue; }

    const refM = line.match(/^((?:R?INV|BNK1)\/\d{4}\/[A-Z0-9]+)\b/i);
    if (!refM) continue;

    let j = i + 1;
    const desc = [line];
    while (j < lines.length && !frEstDateTexte_(lines[j]) && j < i + 10) {
      // Une référence répétée avant la date fait partie de la description OCR.
      desc.push(lines[j]);
      j++;
    }
    if (j + 3 >= lines.length) continue;
    if (!frEstDateTexte_(lines[j]) || !frEstDateTexte_(lines[j+1]) ||
        !frEstMontantTexte_(lines[j+2]) || !frEstMontantTexte_(lines[j+3])) continue;

    const ref = refM[1].toUpperCase();
    const amount = convertirNombre_(lines[j+2]);
    const balance = convertirNombre_(lines[j+3]);
    if (amount === null || balance === null) continue;

    const type = /^BNK1\//i.test(ref) ? 'AJUSTEMENT FOURNISSEUR' :
                 /^RINV\//i.test(ref) ? 'AVOIR' : 'FACTURE';
    entries.push({
      reference: ref,
  ...
  const statementTotal = entries.length ? entries[entries.length-1].balance : 0;
  return {
    ...
    statementTotal: arrondir2_(statementTotal),
    entries: entries
  };
}
...
function frRapprocher_(parsed, bank) {
  ...
  const rows = parsed.entries.map(entry=>{
    if (entry.type !== 'FACTURE' || !(entry.amount > 0)) {
      return {entry:entry, match:{statusCode:'INFO',status:entry.type === 'AVOIR' ? '🟦 Avoir fournisseur' : '🟪 Ajustement fournisseur',score:0,payments:[],paid:0,gap:entry.amount,note:''}};
    }

    const refNorm = frNormaliserReference_(entry.reference);
    const exactRef = supplierPayments.filter(p=>{
      const txt = frNormaliserReference_(p.description + ' ' + p.communication);
      return refNorm && txt.indexOf(refNorm) !== -1;
    });

    let candidates = exactRef.length ? exactRef.slice() : supplierPayments.slice();
    candidates = candidates.map(p=>({p:p,score:frScorePaiement_(entry,p,parsed,!!exactRef.length)}))
      .filter(x=>x.score>=45)
    ...
    if(exactRef.length){
      ...
      if (exactAmount.length) {
        // Priorité absolue au virement portant la bonne référence ET le montant exact.
        // Les autres virements avec la même référence restent visibles dans
        // PAIEMENTS-NON-RAPPROCHES afin de détecter un éventuel double paiement.
        ...
        duplicateRefCount = exactRef.length - 1;
      } else {
        const refCandidates = candidates.filter(x=>frNormaliserReference_(x.p.description+' '+x.p.communication).indexOf(refNorm)!==-1);
        const sumRef = arrondir2_(refCandidates.reduce((s,x)=>s+Math.abs(x.p.amount),0));
        if (refCandidates.length > 1 && Math.abs(sumRef-entry.amount) <= 0.02) selected = refCandidates;
        else if (refCandidates.length) selected = [refCandidates[0]];
    ...
    if(!selected.length){
      return {entry:entry,match:{statusCode:'NONE',status:'❌ Paiement non trouvé',score:0,payments:[],paid:0,gap:entry.amount,note:'Aucun paiement suffisamment fiable trouvé.'}};
    }

    const payments = selected.map(x=>x.p);
    const paid = arrondir2_(payments.reduce((s,p)=>s+Math.abs(p.amount),0));
    const gap = arrondir2_(entry.amount - paid);
    const score = Math.max.apply(null, selected.map(x=>x.score));
    payments.forEach(p=>used[p.id]=true);

    let statusCode='PROB', status='🟠 Paiement probable', note='';
    if (Math.abs(gap) <= 0.02 && score >= 80) {
      statusCode='PAID'; status='✅ Payée';
    } else if (paid > 0 && paid < entry.amount - 0.02 && score >= 70) {
      statusCode='PARTIAL'; status='🔵 Paiement partiel';
    } else if (paid > entry.amount + 0.02 && exactRef.length) {
      statusCode='GROUP'; status='🟡 Paiement groupé / montant supérieur';
      note='La référence de facture est présente, mais le virement est supérieur au montant de cette facture.';
    } else if (score >= 95) {
      statusCode='HIGH'; status='✅ Correspondance très forte';
    }

    const before = reportDate ? payments.filter(p=>p.date <= reportDate) : [];
    const after = reportDate ? payments.filter(p=>p.date > reportDate) : [];
    if (reportDate && before.length && statusCode !== 'PARTIAL') note += (note?' ':'') + 'Paiement trouvé avant la date du rapport.';
    if (reportDate && after.length) note += (note?' ':'') + 'Paiement trouvé après la date du rapport.';
    ...
    return {entry:entry,match:{statusCode:statusCode,status:status,score:score,payments:payments,paid:paid,gap:gap,beforeCount:before.length,afterCount:after.length,note:note}};
  });
...
function frEcrireRapprochement_(ss, parsed, result) {
  ...
  const matchedCount=result.rows.filter(r=>r.match&&r.match.statusCode!=='NONE'&&r.match.statusCode!=='INFO').length;
  ...
  sh.getRange('G3').setValue('Paiements rapprochés').setFontWeight('bold'); sh.getRange('H3').setValue(sumMatched).setNumberFormat('#,##0.00');
  sh.getRange('J3').setValue('Lignes rapprochées').setFontWeight('bold'); sh.getRange('K3').setValue(matchedCount);
