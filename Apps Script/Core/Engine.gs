 ...
 * - respecte le maxRetries propre à chaque tâche ;
 * - compacte la queue pendant les longs traitements afin de limiter PropertiesService ;
 * - ajoute un diagnostic moteur lisible sans modifier le job ;
...
const GDM_Engine = Object.freeze({
  ...
  // La queue est stockée dans PropertiesService : on enlève régulièrement les tâches
  // COMPLETED/SKIPPED lorsque la configuration KEEP_COMPLETED_TASKS=false.
  QUEUE_COMPACT_THRESHOLD_: 20,
  ...
  runUnlocked_: function(jobId, options, token) {
    ...
    while (processedThisRun < maxTasks) {
      ...
      try {
        ...
        /*
         * Queue.failTask() 2.0 utilisait uniquement QUEUE.MAX_RETRIES et ignorait
         * task.maxRetries. Le moteur applique ici la limite réellement portée par la tâche.
         */
        var failedTask = this.failTaskRespectingLimit_(
          jobId,
          task,
          taskError
        );
        ...
        /*
         * IMPORTANT : State.setLastError() incrémente déjà le compteur errors.
         * L'ancienne combinaison increment(errors:1) + setLastError() doublait donc
         * chaque erreur finale. Ici on incrémente seulement processed puis on laisse
         * setLastError() comptabiliser l'erreur une seule fois.
         */
        GDM_State.increment(jobId, { processed: 1 });
        errorsThisRun++;
    ...
    /*
     * Ne pas attendre la fin du job pour compacter la queue. Sur un gros Drive,
     * garder des centaines de tâches terminées fait grossir inutilement le JSON
     * réécrit dans PropertiesService à chaque transition.
     */
    try {
      this.maybeCompactQueue_(jobId);
    } catch (ignoredCompactDuringRun) {}
  ...
  /************************************************************************************************
   * ÉCHEC D'UNE TÂCHE EN RESPECTANT task.maxRetries
   ************************************************************************************************/
  failTaskRespectingLimit_: function(
    jobId,
    task,
    error
  ) {
    ...
    var errorObject =
      GDM_Utils.errorToObject(
        error,
        {
          ...
          module: liveTask.module || '',
          action: liveTask.action || '',
          itemId: liveTask.itemId || '',
          itemName: liveTask.itemName || ''
        }
      );

    /*
     * attempts inclut déjà la tentative courante.
     * maxRetries=0 => aucune nouvelle tentative.
     * maxRetries=1 => une seule nouvelle tentative après la première.
     */
    if (attempts <= maxRetries) {

      return GDM_Queue.updateTask(
        jobId,
        liveTask.taskId,
        {
  ...
  /************************************************************************************************
   * COMPACTAGE PROGRESSIF DE LA QUEUE
   ************************************************************************************************/
  maybeCompactQueue_: function(jobId) {
    ...
    if (
      ...
      return {
        compacted: false,
        reason: 'KEEP_COMPLETED_TASKS activé.'
      };
    }
    ...
    if (!meta) {
      return {
        compacted: false,
        reason: 'Queue absente.'
      };
    }

    /*
     * La queue stabilisée expose compactIfNeeded() et distingue les compteurs
     * cumulés des tâches encore physiquement présentes. On l'utilise en priorité
     * afin de ne pas recompacter inutilement à chaque lot.
     */
    if (typeof GDM_Queue.compactIfNeeded === 'function') {
      var smartResult = GDM_Queue.compactIfNeeded(
        jobId,
        this.QUEUE_COMPACT_THRESHOLD_
      );

      return {
        compacted: !(smartResult && smartResult.skipped === true),
        result: smartResult
      };
    }

    var removable =
      Number(meta.liveCompleted || meta.completed || 0) +
      Number(meta.liveSkipped || meta.skipped || 0) +
      Number(meta.liveCancelled || 0) +
      (
        GDM_Config.get(
          'QUEUE.KEEP_ERROR_TASKS',
          true
        ) === false
          ? Number(meta.liveErrors || meta.errors || 0)
          : 0
      );
  ...
  validate: function() {
    ...
    return {
      ...
      perTaskMaxRetries: true,
      progressiveQueueCompaction: true,
      queueCompactThreshold: this.QUEUE_COMPACT_THRESHOLD_,
...
/**************************************************************************************************
 * DIAGNOSTIC MOTEUR 2.1
 *
 * Lecture seule : ne modifie ni la queue ni les fichiers Drive.
 **************************************************************************************************/
function GDM_engineHealthCheck() {
  ...
  if (jobId) {
    ...
    try {
      token = GDM_Engine.getRunTokenInfo_(jobId);
    } catch (ignoredToken) {}
  }
  ...
  return {
    ...
    queue: queue,
    runToken: token,
    engineTriggers: triggers,
    validation: GDM_Engine.validate()
  };
}
